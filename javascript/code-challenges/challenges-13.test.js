...
const standardizePhoneNumbers = (arr) => {
  ...
  let newArr = [];
  arr.forEach(item=>{
    newArr.push(item.substring(1, 4) +item.substring(6, 9) +item.substring(10, 14) )
  })
  return newArr
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let methode = str.split('');
  let newArr = methode.map((item, idx) => {
    if (idx % 2 == 1) {
      return item;
    }
  })
...
const allHappy = (arr) => {
  ...
  let array =((arr.filter(result => !(result.includes(':)'))).length > 0)?false : true);
  return array
};
